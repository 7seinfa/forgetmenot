...
class _PeopleState extends State<People> {
  ...
  Future<void> loadInfo() async {
    ...
    File pinnedFile = File('$path/people.json');
          ...
          SliverAppBar(
            ...
            title: Padding(
              child: Row(
                ...
                children: [
                  Text("People", style: Theme.of(context).textTheme.headline6.copyWith(fontSize: MediaQuery.of(context).size.width*0.08, color: MyColors.appbarText()), overflow: TextOverflow.visible,),
                ]
              ),
              ...
              builder: (BuildContext context, AsyncSnapshot<bool> snapshot) {
                ...
                if(snapshot.data){
                  for (var x = 0; x<names.length; x++){
                    Image img;
                    print(imagePaths[x]);
                    if(imagePaths[x]!=null&&imagePaths[x].length>0){
                      try{
                    ...
                    people.add(
                      InkWell(
                                  ...
                                  actions: [
                                    ...
                                    FlatButton(
                                      ...
                                      onPressed: () async {
                                        ...
                                        for(var x = 0; x<names.length; x++){
                                          newPeople.add([names[x], numbers[x], descriptions[x]]);
                                        }
                                        File pinnedFile = File('$path/people.json');
                        ...
                        child: FamilyMemberCard(
                          ...
                          image: imagePaths[x]!=null?imagePaths[x].length>0?img:null:null
                        )
                      )
                    );
                  }
                                  ...
                                  return StatefulBuilder(
                                      builder: (BuildContext context, StateSetter setState) {
                                        return Column(
                                          mainAxisSize: MainAxisSize.min,
                                          children: [
                                            Text("Name", style: TextStyle(fontSize: MediaQuery.of(context).size.width*0.06, color: MyColors.selected(), fontWeight: FontWeight.bold),),
                                            TextField(
                                              controller: name,
                                            ...
                                            Text("Phone Number", style: TextStyle(fontSize: MediaQuery.of(context).size.width*0.06, color: MyColors.selected(), fontWeight: FontWeight.bold),),
                                            TextField(
                                              controller: number,
                                              style: TextStyle(color: MyColors.cardText()),
                                              cursorColor: MyColors.gradientLeft(),
                                ...
                                FlatButton(
                                  ...
                                  onPressed: () async {
                                    try{
                                      ...
                                      if(_image!=null&&imageChosen=="Image Chosen!"){
                                      ...
                                      List<List<String>> newPeople = [];
                                      for(var x = 0; x<names.length; x++){
                                        newPeople.add([names[x], numbers[x], descriptions[x], imagePaths[x]]);
                                      }
                                      File pinnedFile = File('$path/people.json');
                                      pinnedFile.writeAsString(json.encode(newPeople));
                                      setState(() {
                            ...
                            child: Row(
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    Text("Add New Person", style: TextStyle(fontSize: MediaQuery.of(context).size.width*0.05, color: MyColors.cardText()), textAlign: TextAlign.left,),
                                  ],
                                )
                              ],
                            )
